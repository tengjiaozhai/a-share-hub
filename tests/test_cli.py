...
def runtime_store(tmp_path):
    ...
    return RuntimeStore(engine, SYSTEM_TENANT)
...
def test_run_decide_command_persists_decision_and_target_position(runtime_store):
    ...
    record = runtime_store.get_decision_run(decision_run_id=summary["decision_run_ids"][0])
...
def test_run_decide_command_is_blocked_when_kill_switch_active(runtime_store):
    SystemRuntimeStore(runtime_store.engine).set_kill_switch(True)
...
def test_run_halt_command_records_kill_switch_event(runtime_store):
    ...
    summary = run_halt_command(reason="manual gate", resume=False, store=system_store)

    assert summary["status"] == "ok"
    assert summary["active"] is True
    assert system_store.get_kill_switch() is True
...
def test_run_halt_command_can_resume(runtime_store):
    system_store = SystemRuntimeStore(runtime_store.engine)
    system_store.set_kill_switch(True)

    summary = run_halt_command(reason="resume gate", resume=True, store=system_store)

    assert summary["status"] == "ok"
    assert summary["active"] is False
    assert system_store.get_kill_switch() is False
